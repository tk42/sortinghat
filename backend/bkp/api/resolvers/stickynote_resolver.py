...
from api.helpers.helper import get_only_selected_fields, get_valid_data
from api.db.session import get_session
from api.models import stickynotes_model, user_model
from api.scalars.stickynotes_scalar import StickyNotes, StickyNotesDeleted, StickyNotesNotFound
from api.scalars.user_scalar import UserNotFound
...
async def get_stickynotes(info):
    ...
    async with get_session() as s:
        sql = (
            select(stickynotes_model.StickyNotes)
...
async def get_stickynote(stickynote_id, info):
    ...
    async with get_session() as s:
        sql = (
            select(stickynotes_model.StickyNotes)
            .options(load_only(*selected_fields))
            .filter(stickynotes_model.StickyNotes.id == stickynote_id)
            .order_by(stickynotes_model.StickyNotes.id)
        )
...
async def update_stickynotes(stickynote_id, text):
    ...
    async with get_session() as s:
        ...
        query = (
            update(stickynotes_model.StickyNotes)
            .where(stickynotes_model.StickyNotes.id == stickynote_id)
            .values(text=text)
        )
        await s.execute(query)
